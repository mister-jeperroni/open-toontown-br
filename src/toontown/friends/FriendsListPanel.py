...
class FriendsListPanel(DirectFrame, StateData.StateData):
    ...
    def enter(self):
        ...
        if self.panelType == FLPNearby:
            taskMgr.doMethodLater(
                5.0, self.__updateNearbyPlayers, "updateNearbyPlayers"
            )
        return
    ...
    def exit(self):
        ...
        # Remover a tarefa quando sair do painel
        taskMgr.remove("updateNearbyPlayers")
        base.localAvatar.obscureFriendsListButton(-1)
    ...
    def __updateScrollList(self):
        ...
        if self.panelType == FLPNearby:
            for av in list(base.cr.doId2do.values()):
                ...
                if (
                    av.__class__ == DistributedToon.DistributedToon
                    and av.doId != base.localAvatar.doId
                ):
                    nearbyAvatars.append((av.doId, 0))
        ...
        nearbyAvatars.sort(key=functools.cmp_to_key(compareFriends))
        ...
        for friendPair in nearbyAvatars:
            if friendPair not in self.friends:
                if friendPair in base.localAvatar.friendsList:
                    friendButton = self.makeFriendButton(
                        friendPair, ToontownGlobals.ColorSpeedChat, 0
                    )
                else:
    ...
    def __updateTitle(self):
        if self.panelType == FLPOnline:
            ...
            self.title["text"] = TTLocalizer.FriendsListPanelNearbyPlayers
        else:
    ...
    def __updateNearbyPlayers(self, task):
        if self.panelType == FLPNearby:
            self.__updateScrollList()
        return task.again
